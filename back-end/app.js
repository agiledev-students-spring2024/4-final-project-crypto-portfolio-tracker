...
app.get("/api/portfolios", async (req, res) => {
  try {
    ...
    const updatedPortfolios = await Promise.all(portfoliosData.map(async (portfolio) => {
      ...
      let confirmedBalance = chain_stats.funded_txo_sum - chain_stats.spent_txo_sum;
      let unconfirmedBalance = mempool_stats.funded_txo_sum - mempool_stats.spent_txo_sum;
    ...
    res.status(500).json({ message: 'Error fetching portfolio data with balance' });
  }
});
